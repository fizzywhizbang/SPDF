...
do
    case "${flag}" in
        ...
        echo "Usage:"
        echo "-o output name"
        echo "-a will run the whole process"
        echo "-i will create images"
        echo "-p will make PDFs"
        echo "-s will scale the PDFs to A4 so they all match in size"
        echo "-m will merge the pdfs into one final container specified in the -o parameter"
        echo "-r will use ghostscript to resize the reultant container"
        echo "#####################################################"
        exit 1
        ;;
    esac
...
printf "OUTPUT Name: ${Green} ${OUTPUT} ${NC}\n"


if [ $RUNALL == TRUE ]
then
    echo "Running the whole shebang"
